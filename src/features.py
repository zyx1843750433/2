...
def build_feature_frame(df: pd.DataFrame) -> Tuple[pd.DataFrame, list[str]]:
    ...
    # Keep these columns for downstream logic/diagnostics (idle detection etc.)
    # (They no longer participate as regression features.)
    out["cpu"] = out["cpu_usage"]
    out["cpu_freq"] = out["cpu_freq_mean"]
    ...
    # Keep screen_on as original; only change CPU part to multiplicative structure.
    ...
    # CPU (changed): replace (cpu, cpu_freq) with an interaction feature
    feats["cpu_eff"] = out["cpu_usage"] * out["cpu_freq_mean"]
