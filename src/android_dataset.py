...
def load_background_app_count(index: DatasetIndex, date: str, device_id: str) -> Optional[pd.DataFrame]:
    ...
    def count_apps(x: Any) -> float:
        """Parse the background_apps field into a numeric count.

        In this dataset, the value is usually a semicolon-separated list, and '-' means
        "no background apps" (0), NOT "missing". Treating it as missing and forward-filling
        can wrongly propagate a large count across long time ranges.
        """
...
def merge_background(dynamic_df: pd.DataFrame, bg_df: Optional[pd.DataFrame]) -> pd.DataFrame:
    ...
    If bg_df is None, create bg_app_count=NaN and later fill to 0.
    """
